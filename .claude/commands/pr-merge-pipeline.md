...
for issue_num in {issue_numbers}; do
  gh pr list --repo Kewton/CommandIndex --head "feature/issue-${issue_num}" --json number,title,state
done
```
...
```bash
for issue_num in {issue_numbers}; do
  PR_NUM=$(gh pr list --repo Kewton/CommandIndex --head "feature/issue-${issue_num}" --json number -q '.[0].number')
...
```bash
for each PR:
  gh pr checks <PR_NUM> --repo Kewton/CommandIndex
```
...
```bash
for each PR:
  gh pr checks <PR_NUM> --repo Kewton/CommandIndex --watch
```
...
1. **各PRの変更規模を取得**:
   ```bash
   for each PR:
     gh pr diff <PR_NUM> --repo Kewton/CommandIndex --stat
   ```
...
```bash
gh pr view <PR_NUM> --repo Kewton/CommandIndex --json mergeable,mergeStateStatus
```
...
commandmatedev send <worktree-id> \
  ...
  解消後:
  1. cargo build && cargo clippy --all-targets -- -D warnings && cargo test --all で確認
...
```bash
gh pr merge <PR_NUM> --merge --repo Kewton/CommandIndex
```
...
- [ ] 全PRがdevelopにマージ済み
- [ ] マージ後のビルド・テストが全パス
- [ ] 最終統合検証（build, clippy, test, fmt）が全パス
- [ ] 結果レポートが出力されている

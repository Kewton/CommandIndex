...
1. cargo fmt --all
2. cargo clippy --all-targets -- -D warnings（警告0件であること）
3. cargo test --all（全テストパスであること）
4. 修正内容をコミット
5. git push
```
...
```bash
for issue_num in {issue_numbers}; do
  gh pr list --repo Kewton/CommandIndex \
...
for each PR:
  # コンフリクト確認
  gh pr view <PR_NUM> --repo Kewton/CommandIndex --json mergeable
...
commandmatedev send <worktree-id> \
  ...
  解消後 cargo build && cargo test --all で確認し、git push --force-with-lease してください" \
  --auto-yes --duration 1h

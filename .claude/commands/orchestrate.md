...
for issue_num in {issue_numbers}; do
  gh issue view "$issue_num" --repo Kewton/CommandIndex --json number,title,body,labels
done
```
...
git worktree add -b "feature/issue-{N}-{description}" "../CommandIndex-feature-issue-{N}-{description}" develop
```
...
```bash
for each issue:
  commandmatedev send <worktree-id> "/pm-auto-issue2dev {issue_number}" \

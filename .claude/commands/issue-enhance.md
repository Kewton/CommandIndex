...
- `src/search/` - 検索ロジック
- `src/output/` - 出力フォーマット（human / json / path）
...
Q1（背景・課題が不足時）:
  ...
  options:
    ...
    - label: "{コードベース調査から推測される背景2}"
      description: "{詳細}"
    - label: "検索体験の改善"
      description: "ローカルナレッジ検索の体験を向上させるため"
    - label: "技術的制約の解消"
...
Q3（受入条件が不足時）:
  ...
  options:
    ...
    - label: "テスト必須"
      description: "cargo testで全テストパス必須"
    - label: "パフォーマンス要件あり"
      description: "検索レスポンス時間等の要件がある"
...
Q4（影響範囲が不足時）:
  ...
  options:
    ...
    - label: "インデックス構造への影響"
      description: "tantivy/SQLiteインデックスの構造変更を伴う"
    - label: "パーサーへの影響"
      description: "Markdown・ソースコード解析に影響がある"
    - label: "CLI互換性"
      description: "既存のCLIインターフェースに影響がある"
```
...
Q1（再現手順が不足時）:
  ...
  options:
    ...
    - label: "特定のファイル形式のみ"
      description: "特定のMarkdownやソースコードでのみ発生"
...
Q3（実際の動作が不足時）:
  ...
  options:
    ...
    - label: "検索結果が不正"
      description: "検索結果が期待と異なる・欠落している"
    - label: "機能しない"
      description: "操作しても反応がない/動作しない"
    - label: "インデックスの不整合"
      description: "インデックス作成・更新時に問題が発生する"
...
更新URL: https://github.com/Kewton/CommandIndex/issues/{issue_number}
